...
export class TrackStrategy implements OnInit {
  readonly trackId=input.required<string>();private readonly http=inject(HttpClient);readonly error=signal('');readonly percentage=signal<number|null>(null);readonly ready=signal(false);readonly busy=signal(false);readonly saved=signal(false);stages:{name:string;internal:boolean}[]=[];revision=0;
  ngOnInit(){this.load();}
