...
interface CapitalState { pools: Pool[]; roles: string[]; allocated: number; liquid: number | null; knownLiquid: number; overAllocated: number | null; walletCount: number; characterCount: number; walletsStale: boolean; adjustments: {id:string; date:string; fromName:string; toName:string; amount:number; reason:string; notes:string}[]; }
@Component({selector:'app-capital', imports:[FormsModule, DatePipe, DecimalPipe, MatButtonModule], template:`
  ...
  @if (state(); as state) {
    ...
    <section class="panel"><div class="section-heading"><h2>Pools</h2><button mat-stroked-button (click)="reset()">New pool</button></div><div class="table-wrap"><table><thead><tr><th>Pool</th><th>Programme role</th><th>Allocated ISK</th><th>Target ISK</th><th></th></tr></thead><tbody>
      @for (pool of state.pools; track pool.id) { <tr><td>{{ pool.name }}<small>{{ pool.description }}{{ pool.archivedAt ? ' · Archived' : '' }}</small></td><td>{{ pool.role }}</td><td>{{ pool.allocatedCapital | number:'1.2-2' }}</td><td>{{ pool.targetCapital === null ? 'Not set' : (pool.targetCapital | number:'1.2-2') }}</td><td><button mat-button (click)="edit(pool)">Edit {{ pool.name }}</button></td></tr> } @empty { <tr><td colspan="5">No pools yet. Create a pool, then allocate capital below.</td></tr> }
