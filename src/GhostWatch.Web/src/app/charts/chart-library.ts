...
<p class="eyebrow">ECONOMICS / VISUALISATION</p><div class="page-heading"><h1>Chart library</h1><button mat-flat-button [disabled]="!schema()" (click)="reset()">New chart</button></div><p class="muted">Definitions describe the data. Dashboard and Track placements reuse them with independent widths and ordering.</p>
...
@if(!schema()&&!error()){<p role="status">Loading chart library…</p>}
@if(schema()){
