...
@Component({selector:'app-run-detail',imports:[FormsModule,RouterLink,DecimalPipe,MatButtonModule,IndustryJobs],template:`
  ...
  @if(ready()){
    ...
    <form class="panel compact-form" #form="ngForm" (ngSubmit)="save()">
      <label>Playbook<select name="playbook" [(ngModel)]="draft.playbookId"><option [ngValue]="null">None</option>@for(book of playbooks();track book.id){<option [ngValue]="book.id">{{book.name}}</option>}</select></label>
...
export class RunDetail {
  ...
  load(){
    this.error.set('');
    forkJoin({playbooks:this.http.get<{id:string;name:string}[]>('/api/knowledge/playbooks'),options:this.http.get<RunOptions>('/api/economics/runs/options'),tracks:this.http.get<Track[]>('/api/economics/tracks?includeArchived=true'),capital:this.http.get<{pools:Pool[]}>('/api/economics/capital'),view:this.id?this.http.get<RunView>(`/api/economics/runs/${this.id}`):of(null),jobs:!this.id&&this.route.snapshot.queryParamMap.has('jobId')?this.http.get<IndustryJob[]>('/api/eve/industry-jobs'):of([])}).subscribe({next:result=>{
