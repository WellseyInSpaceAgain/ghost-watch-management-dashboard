...
test('chart definitions preview, share across pages and retain dragged layout widths',async({page,request})=>{
 ...
 const run=await request.post('/api/economics/runs',{data:{run:{name:'Chart batch',trackId:track.id,runType:'Manufacturing',purpose:'Commercial',status:'Completed',startedAt:new Date(Date.now()-86400000).toISOString(),completedAt:new Date().toISOString(),actualInputCost:100,actualJobCost:0,actualOtherCost:0,actualRevenue:150,expectedInputCost:100,expectedJobCost:0,expectedOtherCost:0,expectedRevenue:175,productName:'Shield',verdict:'Scale',notes:''}}});expect(run.ok()).toBe(true);
