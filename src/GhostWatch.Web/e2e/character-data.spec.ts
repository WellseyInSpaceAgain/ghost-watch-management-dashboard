...
test('refresh shows progress and retains previous wallet on partial failure', async ({ page }) => {
  ...
  await page.route('**/api/eve/characters/7/data', route => route.fulfill({ json: {
    ...
    jobs: [{ jobId: 42, activityId: 1, productTypeId: 456, blueprintTypeId: 123, productName: 'Raven', blueprintName: 'Raven Blueprint', runs: 1, status: 'active', endDate: '2026-09-23T00:00:00Z', lastSeenAt: '2026-09-22T00:00:00Z' }],
  } }));
  ...
  await expect(page.getByRole('cell', { name: 'Raven Type 456' })).toBeVisible();
