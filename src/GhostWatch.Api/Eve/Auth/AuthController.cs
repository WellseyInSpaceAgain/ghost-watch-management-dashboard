    ...
    IOptions<EveOptions> options, GhostWatchDbContext db, CharacterGate gates, ILogger<AuthController> logger) : ControllerBase
{
    ...
    {
        if (!options.Value.Configured) return Outcome("configuration");
        try
        {
    ...
    public async Task<IActionResult> Callback(string? code, string? state, string? error, CancellationToken ct)
    {
        ...
        if (error is not null || string.IsNullOrWhiteSpace(code)) return Outcome("cancelled");
        try
        {
    ...
    private IActionResult Failed(Exception ex)
    {
    ...
    private RedirectResult Outcome(string result) => Redirect($"/characters?auth={result}");
